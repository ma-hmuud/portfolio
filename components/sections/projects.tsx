...
export function Projects() {
  return (
    <section id="work" className="py-32 relative bg-background border-t-4 border-foreground">
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-6 mb-4 md:mb-6">
            <h2 className="text-5xl sm:text-6xl md:text-8xl font-black uppercase tracking-tighter">Selected <span className="text-accent">Work</span></h2>
              ...
              <div className="grid lg:grid-cols-12 gap-0 brutal-border brutal-shadow bg-card hover:translate-x-1 hover:-translate-y-1 transition-transform duration-200">
                ...
                <div className="lg:col-span-5 p-6 sm:p-8 md:p-12 flex flex-col justify-between border-b-4 lg:border-b-0 lg:border-r-4 border-foreground">
                  <div>
                    <div className="flex flex-wrap items-center justify-between gap-4 mb-6 md:mb-8">
                      ...
                      {project.featured && (
                        <div className="flex items-center font-bold text-accent font-mono text-xs md:text-sm uppercase tracking-widest bg-accent/10 px-2 py-1 md:px-3 md:py-1 brutal-border">
                          <Star className="w-3 h-3 md:w-4 md:h-4 mr-1 md:mr-2 fill-current" /> Featured
                        </div>
                      )}
                    </div>
                    ...
                    <ul className="space-y-3 md:space-y-4 mb-6 md:mb-8">
                      {project.bullets.map((bullet, i) => (
                        <li key={i} className="flex items-start gap-2 md:gap-3 font-mono text-xs md:text-sm leading-relaxed border-l-2 border-accent pl-2 md:pl-3">
                          {bullet}
                        </li>
                      ))}
                    </ul>
                  ...
                  <div className="flex flex-col sm:flex-row flex-wrap gap-4 mt-auto">
                    {project.live !== "#" && (
                      <Button asChild size="lg" className="w-full sm:w-auto rounded-none brutal-border brutal-shadow bg-accent text-accent-foreground hover:bg-foreground hover:text-background transition-none">
                        <a href={project.live} target="_blank" rel="noopener noreferrer">
                          VISIT SITE <ArrowUpRight className="ml-2 h-4 w-4 md:h-5 md:w-5" strokeWidth={3} />
                        </a>
                      </Button>
                    )}
                    {project.github !== "#" && (
                      <Button asChild variant="outline" size="lg" className="w-full sm:w-auto rounded-none brutal-border brutal-shadow bg-background text-foreground hover:bg-foreground hover:text-background transition-none">
                        <a href={project.github} target="_blank" rel="noopener noreferrer">
                          <Github className="mr-2 h-4 w-4 md:h-5 md:w-5" strokeWidth={3} /> SOURCE
                        </a>
                      </Button>
                    )}
                  </div>
