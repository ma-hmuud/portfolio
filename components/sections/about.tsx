...
import { Briefcase, Code2, GraduationCap, HeartHandshake } from "lucide-react";
...
const experience = [
  {
    ...
    type: "Freelance",
    description: "Architected RESTful APIs with Express.js, PostgreSQL, Redis, and Drizzle ORM handling 500+ daily requests. Designed 10+ schemas and integrated AWS S3/SendGrid. Collaborated on Next.js frontend, improving load time by 40%."
  },
  {
    ...
    type: "Training",
    description: "Developed ASP.NET Core full-stack apps. Co-led a 3-person team building an educational platform for 200+ users. Optimized SQL Server queries, reducing load times by 30%."
  },
];
...
const volunteer = {
  ...
  description: "Mentored 50+ students in Java and problem-solving. Authored 5+ Java OOP workshops.",
};
...
export function About() {
  return (
    <section id="about" className="py-32 relative bg-background border-t-4 border-foreground overflow-hidden">
        ...
        <div className="grid lg:grid-cols-2 gap-24 items-start">
          ...
          >
            ...
            <h2 className="text-4xl sm:text-5xl md:text-7xl font-black uppercase mb-8 md:mb-10 leading-[1] md:leading-[0.9]">
              ENGINEERING WITH <br className="hidden sm:block" /> <span className="text-stroke">PRECISION</span> AND RAW POWER.
            </h2>
            ...
            <div className="space-y-4 md:space-y-6 font-mono text-base md:text-lg text-muted-foreground leading-relaxed border-l-4 border-foreground pl-4 md:pl-6">
              <p>
                I don&apos;t just write code; I engineer solutions that are scalable, maintainable, and visually striking.
              </p>
              <p>
                Currently finishing my degree in Data Science (3.6 GPA), I blend academic theory with messy, real-world execution.
              </p>
              <div className="bg-foreground text-background p-4 md:p-6 brutal-shadow brutal-border mt-6 md:mt-8">
                <h4 className="font-black text-xl mb-2 text-accent">VOLUNTEER COMMAND</h4>
                <p className="font-bold">{volunteer.role} @ {volunteer.organization}</p>
                <p className="text-sm mt-2 opacity-80">{volunteer.description}</p>
              </div>
            </div>
          </motion.div>
          ...
          <div className="space-y-16 md:space-y-24 mt-12 lg:mt-0">
            ...
            >
              <div className="flex items-center gap-3 md:gap-4 mb-8 md:mb-12">
                ...
                <h3 className="text-3xl md:text-4xl font-black uppercase tracking-tighter">Experience</h3>
              </div>

              <div className="space-y-6 md:space-y-8">
                {experience.map((exp, i) => (
                  <div key={i} className="brutal-card p-5 sm:p-6 md:p-8 bg-card hover:bg-muted transition-colors brutal-shadow">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 md:mb-6 gap-3 md:gap-4">
                      <div>
                        <h4 className="font-black text-xl md:text-2xl uppercase">{exp.role}</h4>
                        <p className="font-mono font-bold text-accent text-sm md:text-base">{exp.company}</p>
                      </div>
            ...
            >
              ...
              <div className="brutal-card p-8 bg-card brutal-shadow">
                <h4 className="font-black text-2xl uppercase">{education.degree}</h4>
                <p className="font-mono font-bold text-accent mt-2">{education.institution}</p>
                <div className="flex flex-col gap-2 mt-4 font-mono text-sm">
                  <span className="bg-muted inline-block px-3 py-1 brutal-border w-fit">{education.details}</span>
                  <span className="text-muted-foreground font-bold">{education.period}</span>
                </div>
              </div>
            </motion.div>
            ...
            >
              <div className="flex items-center gap-4 mb-12">
                ...
                <h3 className="text-4xl font-black uppercase tracking-tighter">Arsenal</h3>
              </div>

              <div className="space-y-8">
                {skillCategories.map((category) => (
                  <div key={category.name} className="border-l-4 border-accent pl-4">
                    <h4 className="font-black text-xl uppercase mb-4">{category.name}</h4>
